...
{
    std::cout << executor.getName() << " : Default action called. This should not happen." << std::endl;
}
...
void        AForm::execute(const Bureaucrat & executor) const 
{
        ...
        {
            std::cout << executor.getName() << " couldn't execute the form \"" << this->name 
                        << "\" as it wasn't signed !" << std::endl;
            throw FormUsignedException();
        }
        if (executor.getGrade() > this->toExec)
        {
            std::cout << executor.getName() << " couldn't execute the form \"" << this->name 
                        << "\" because he had a too low grade !" << std::endl;
            throw GradeTooLowException();
        }
        std::cout << this->name << " was executed by \"" << executor.getName() << "\"." << std::endl;
